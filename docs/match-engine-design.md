...
PR89 introduces `decision/CarryOptionEvaluator` and `decision/ShotDecisionEvaluator` so the three ball-carrier action categories now have focused decision-layer components. It also adds `decision/BallCarrierDecisionModel` as the central combination layer for pass, carry, shot, hold, and emergency clear options. The old candidate generator was removed so it can no longer drift into the tactical decision brain.
...
`PossessionContext` now computes progression and circulation availability from current teammate positions, opponent pressure, and simple lane pressure rather than static placeholder values. It also estimates opponent block compactness from nearby defender density and spread around the ball. Short-pass loop prevention is therefore handled through context and defensive response: long non-progressive possession can make the defending team more compact and locally active, which changes pressure and lane quality, without pass-count score penalties.

Possession percentage remains time-based controlled-possession accounting. Neutral loose-ball time is not credited to the previous team and is excluded from the home/away possession denominator. Pass-count ratios and possession-share ratios are therefore related but not expected to match exactly. The existing `actionDepth` field is used as `possessionActionCount`: it increments only when a controlled ball-carrier decision is selected/executed, not during in-flight travel, loose-ball waits, deflection continuation, save processing, restart delay, or movement-only steps.
...
`BallCarrierDecisionModel` is the central ball-carrier orchestration layer. It builds evaluator contexts, calls `PassOptionEvaluator`, `CarryOptionEvaluator`, and `ShotDecisionEvaluator`, then delegates candidate construction to `ActionScoringModel`. It no longer owns a hidden category-suitability layer or multiplies option scores by a second set of pass/carry/shoot boosts.

`ActionScoringModel` converts evaluator DTOs into `ActionCandidate` values through an explicit `ActionScoreBreakdown`: retention value, progression value, chance value, tactical fit, role fit, skill fit, pressure cost, turnover-risk cost, phase fit, and final score. Evaluators remain responsible for option facts and option-level quality fields; the scoring model is responsible for turning those facts into comparable candidate scores. Decision profile values, role/tactical preferences, pass/carry/shot action scoring profiles, hold/clear scoring profiles, evaluator scoring baselines, and selector sharpness/skill influence now live in `DecisionTuningProfile.h` rather than being spread through the coordinator or evaluator implementations.

Defensive response now affects pass execution rather than just intent labels. Pressing, containing, blocking, covering, and marking defenders can increase contextual pass pressure and feed `PassResolutionFlow` / `PassResolutionModel` as the authoritative result path for pass-like actions. That pass-resolution seam evaluates receiver reach, defender lane/arrival/receiver relevance, execution quality, pressure, and defender reaction timing before choosing clean receive, contested receive, defender intercept, deflected loose ball, or misplaced loose ball. A defensive intervention cannot also become completed-pass credit on the same pass. Repeated short circulation becomes less comfortable because defenders move, contest lanes, and can create real turnovers or loose-ball recovery races, not because a global counter punishes safe passes.

Loose-ball recovery is handled separately by `LooseBallRecoveryModel`. A genuinely loose or misplaced ball clears the controlled carrier and controlled team; the previous possession/touch team is kept separately only as `lastPossessionTeamId` for traces, restarts, and recovery context. Recovery considers nearby players from both teams, distance, pace/acceleration, current recovery/pressing intent, and recent possession context. If nobody can control immediately, the ball remains loose for another short step rather than being automatically returned to the previous possession team.
...
Shooting now uses separate seams for the decision, intended target, execution, trajectory, block, save, and rebound phases. `ShotDecisionEvaluator` only decides whether the current situation deserves a shot option and exposes a neutral goal-center preview point for generic action scoring. The real intended lane/height comes from `ShotTargetSelector`, while `ShotExecutionModel` turns that intention into the actual target/path quality used by trajectory and outcome resolution.
...
The selector remains weighted and deterministic. Evaluators and `ActionScoringModel` score options; `ActionSelector` still turns candidate scores into a deterministic weighted choice where higher scores are more likely, better Decisions sharpen selection, lower Decisions allow more variance, and the top score is not automatically chosen. Its sharpness and action-skill influence constants are named in the central decision tuning profile.

The next phase should focus on the dedicated shot/xG/shot-outcome overhaul: shot selection, chance-quality calibration, shot targeting, goalkeeper/defender block logic, and richer conversion/save outcomes. Future work should also deepen phase modeling, real rebounds/corners/fouls, pressing/marking, and full `ActionPlan` reassessment.
...
Each action step wires the existing helper layers in order: `TeamShapeModel`, `PlayerIntentResolver`, `MovementResolver`, `BallCarrierDecisionModel`, `ActionSelector`, `BallTrajectoryBuilder`, and the relevant ball-resolution seam. Controlled balls can hold, carry/dribble, pass/cross/clear, or shoot into a trajectory. Pass-like in-flight balls resolve through `PassResolutionFlow` / `PassResolutionModel` rather than a parallel legacy interception path; non-pass shots/blocks can still use local contest helpers. Intended receivers still get explicit receive-pass behavior and a dynamic reception range based on pass type, execution quality, first touch/technique/composure, and pressure, but defender reaction is evaluated by geometry and ETA rather than only by previous intent labels. Loose balls are recovered through `LooseBallRecoveryModel`, which compares nearby players from both teams by distance, pace/acceleration, current recovery intent, and recent possession context rather than nearest-player-only assignment.

Shot/save/goal handling exists inside this local simulator. Coordinate goals update `MatchEngineResult` team/player stats, record assists from recent intentional same-team passes where applicable, append official `MatchEventRecord` goal events, then reset the local ball for a simple kickoff after a deterministic restart delay. Watched/debug traces may also record goal markers, but official report events do not depend on trace frames. The simulator does not apply the report; `PlayMatchCommandHandler` validates it and calls `League::applyMatchReport`. First-pass guardrails now gate low-value shot selection, use `ShotQualityModel` to estimate open-play xG from shot location, real goal angle, and pressure, and keep shooter/goalkeeper attributes out of the xG number. Intended shot targeting belongs to `ShotTargetSelector`, actual shot path belongs to `ShotExecutionModel` and `ShotTrajectoryBuilder`, and saved rebounds or blocked deflections travel as explicit uncontrolled ball states before entering loose-ball recovery rather than masquerading as carrier actions.
