...
- Match Engine Coordinate Simulation Design documented in `docs/match-engine-design.md`; the matchSecond/action-duration coordinate simulator is now the default playable runtime match engine.
...
- The playable V1 engine now uses real pitch dimensions, tactical shape, action-duration timing, ball trajectories, path interception, local contests, watched/debug traces, and background summaries.
...
- The next planned implementation phases are watched/background mode separation, tuning/stability, match event richness and UI playback preparation, and eventual lightweight deprecation/cleanup after confidence.
...
22. Eventual lightweight deprecation/cleanup after confidence
23. Multi-league expansion preparation
